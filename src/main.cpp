...
void loop() {
    ...
    for (byte channel = 0; channel <= (channelAmount-1); ++channel) {
        ...
        if(rx[channel]>255) rx[channel]=255;
        else if(rx[channel]<-255) rx[channel]=-255;
        Serial.print(">rx"); 
...
void readMPU() {
    ...
    Serial.print(">AcX:"); Serial.println(a.acceleration.x); 
    Serial.print(">AcY:"); Serial.println(a.acceleration.y); 
    Serial.print(">AcZ:"); Serial.println(a.acceleration.z); 
    
    Serial.print(">GyX:"); Serial.println(g.gyro.x); 
    Serial.print(">GyY:"); Serial.println(g.gyro.y); 
    Serial.print(">GyZ:"); Serial.println(g.gyro.z); 
...
void drive(int speed, int steer)
{
    ...
    else if(speed<speedDeadzone) speed = speed+speedDeadzone;
         else speed = speed-speedDeadzone;
    if((steer<steerDeadzone)&&(steer>-steerDeadzone)) steer = 0;
    else if(steer<speedDeadzone) steer = steer+speedDeadzone;
         else steer = steer-speedDeadzone;
